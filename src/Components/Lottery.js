...
const Lottery = () => {
  ...
  const pullChar = () => {
    ...
    const guaranteedCharacter = lastPullPool[Math.floor(Math.random() * lastPullPool.length)];
  ...
  // Function to reset the gacha and immediately pull again
  const resetChar = () => {
    setResults([]);
    setShowRetry(false);
    pullChar(); // Immediately start a new 10-pull after resetting
  };
  ...
  return (
    <div style={{ textAlign: "center", marginTop: "20px" }}>
      ...
      <Button onClick={() => setIsModalVisible(true)} style={{ marginBottom: "20px" }}>
        <FormattedMessage id="Gacha.Swap" defaultMessage="Swap Characters" />
      </Button>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", justifyContent: "center" }}>
              {Array.from({ length: 20 }, (_, i) => i + 1).map((char) => (
                <img
                  ...
                  alt={`Character ${char}`}
                  style={{
                    border: selectedLeft === char ? "2px solid blue" : "1px solid #ddd",
                    borderRadius: "5px",
                    cursor: "pointer",
                    width: "45px",
                    height: "45px",
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", justifyContent: "center" }}>
              {Array.from({ length: 80 }, (_, i) => i + 21).map((char) => (
                <img
                  ...
                  alt={`Character ${char}`}
                  style={{
                    border: selectedRight === char ? "2px solid blue" : "1px solid #ddd",
                    borderRadius: "5px",
                    cursor: "pointer",
                    width: "45px",
                    height: "45px",
      ...
      {!gameStarted ? (
        ...
        >
          <FormattedMessage id="Gacha.Pull" defaultMessage="Pull 10" />
        </Button>
      ) : (
        <>
          {results.length > 0 && (
            <div style={{ marginTop: "20px" }}>
              ...
              {showRetry && (
                <Button onClick={resetChar} style={{ marginTop: "20px" }}>
                  <FormattedMessage id="Gacha.Retry" defaultMessage="Retry" />
                </Button>
              )}
            </div>
          )}
